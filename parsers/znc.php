...
class parser_znc extends parser
{
	protected function parse_line(string $line): void
	{
		...
		} elseif (preg_match('/^'.$timestamp.'\*\*\* Joins: (?<nick>\S+) \(\S+\)$/', $line, $matches)) {
			$this->set_join($matches['time'], $matches['nick']);
		} elseif (preg_match('/^'.$timestamp.'\*\*\* Quits: (?<nick>\S+) \(\S+\) \(.*\)$/', $line, $matches)) {
		...
		} elseif (preg_match('/^'.$timestamp.'\*\*\* Parts: (?<nick>\S+) \(\S+\)( \(.*\))?$/', $line, $matches)) {
