...
class parser_textual extends parser
{
	protected function parse_line(string $line): void
	{
		...
		} elseif (preg_match('/^'.$timestamp.'(?<nick>\S+) \(\S+\) joined the channel/', $line, $matches)) {
		...
		} elseif (preg_match('/^'.$timestamp.'(?<nick>\S+) \(\S+\) left the channel/', $line, $matches)) {
			$this->set_part($matches['time'], $matches['nick']);
		} elseif (preg_match('/^'.$timestamp.'(?<nick>\S+) changed the topic to (?<line>.+)$/', $line, $matches)) {
			$this->set_topic($matches['time'], $matches['nick'], $matches['line']);
		} elseif (preg_match('/^'.$timestamp.'(?<line>(?<nick_performing>\S+) kicked (?<nick_undergoing>\S+) from the channel\. )(?<reason>\(.*\))$/', $line, $matches)) {
